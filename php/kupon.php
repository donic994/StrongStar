...
<html>
    ...
    <body>
        ...
        <section>
            ...
            $result = $veza->selectDB("SELECT * FROM Kupon");
            $fupit="SELECT * FROM Kupon";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<div style=" . "float:left" . "> 
                <ul id=" . "odabir" . ">
                    <li>
                    <a id="."pdf". $row["idKupon"] ." href="." "."/>
                            <img src=" . "../slike/kupon.png" . " onclick=otvori('". $row["Naziv"] ."','". $row["idKupon"] ."')>
                            <h3 id="."naziv".">". $row["Naziv"] . "</h3>
                                <h4>" . $row["Popust"] . "%</h4>
                                    <h5>" . $row["Potrebni_bodovi"] . " bodova</h5>
                            <img id="."slika". $row["idKupon"] ." src=" . "../slike/kosarica.png" . " onclick=Dodaj_u_kosaricu('". $row["Naziv"] ."','". $row["idKupon"] ."');>
                    </li>
                </ul>
            </div>";
    }
